...
import { IconAdd, IconEdit, IconRemove, IconFolder, IconFullScreen } from './Icons';
...
export default class FileInput extends Component {
	...
	_fileHandler (indicator, e) {
		...
		var setState = function () {
			this.setState({
				...
				if (props.onChange) {
					if (props.multiple) props.onChange(fileList, props.name);
					else props.onChange(fileList[0], props.name);
				}
			})
		}.bind(this);
	...
	_removeFile (index, e) {
		...
		var setState = function () {
			this.setState({
				...
				if (onChange) {
					if (props.multiple) onChange(fileList, props.name);
					else onChange([], props.name);
				}
			})
		}.bind(this);
	...
	_showFile (index, e) {
		...
		if (typeof file == 'string') {
			this.refs['file-viewer'].showFile({file: {url: file}});
		} else {
			this.refs['file-viewer'].showFile({file: file});
		}
	}
		...
		var createFilePreviewerList = function (file, index) {
			...
			return (
				<div key={index} style={{display: 'inline-block', margin: '10px', padding: '10px', position: 'relative'}}>
					<div style={{position: 'absolute', right: '-6px', top: '-11px'}} title="Remove File">
						<IconRemove onClick={self._removeFile.bind(self, index)} />
					...
					<div style={{position: 'absolute', left: '-11px', top: '-10px'}} title="Full Screen">
						<IconFullScreen onClick={self._showFile.bind(self, index)} style={{fill: '#7b7373', height: '18px', width: '18px'}}/>
						{/*<i onClick={self._removeFile.bind(self, index)} className="fa fa-remove" style={{cursor: 'pointer', verticalAlign: 'middle', fontSize: '1.2em', lineHeight: "22px", color: '#c0582b'}}></i>*/}
					</div>
				</div>
			);
				...
				<div className="input-group" style={{borderRadius: '0px 0px 4px 0px'}}>
					...
					<div className="input-group-btn">
						...
						<div className="btn btn-primary btn-file" style={{height: '34px', lineHeight: '16px', borderRadius: !fileListLen ? '0px 4px 4px 0px' : multiple ? '0px' : '0px 0px 4px 0px', marginRight: '-1px', position: 'relative', zIndex: 2}}>
							{fileListLen ?
								<IconEdit style={{marginTop: '-4px'}} />
									:
								<IconFolder style={{marginTop: '-4px'}} />
							}
